...
impl Counter {
    ...
    fn trim_sequence<'a>(record: &'a Record, offset: usize, size: usize) -> &'a str {
        if record.seq().len() < offset + size {
            panic!("Cannot trim record to required size: {}", record.seq())
        }
        match std::str::from_utf8(&record.seq().as_bytes()[offset..offset+size]) {
            Ok(trim) => trim,
            Err(why) => panic!("{}", why)
        }
    }
...
mod test {
    ...
    fn count_no_distance_no_permute() {
        ...
        let library = library();
        let count = Counter::new(trimmer, &library, &None, 0, 4);
    ...
    fn count_with_distance_with_permute() {
        ...
        let count = Counter::new(trimmer, &library, &Some(permuter), 0, 4);
        assert_eq!(*count.get_value("seq.0"), 1);
    }
}
